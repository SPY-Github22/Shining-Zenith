...
// Build dynamic prompt based on what info we already have
...
// AGGRESSIVE extraction - capture everything possible
function extractIntelligence(messages) {
    ...
    // ===== NAME EXTRACTION (more lenient) =====
    // Capture any capitalized word after name indicators
    const namePatterns = [
        /(?:my name is|i am|i'm|this is|call me|it's|speaking)\s+([A-Z][a-zA-Z]+)/gi,
        /(?:name is|name's)\s+([A-Z][a-zA-Z]+)/gi,
        // Capture standalone capitalized names at start of sentences
    ...
    // ===== UPI ID (lenient) =====
    // Capture anything that looks like id@provider
    ...
    extracted.upiIds = [...new Set(upiMatches.filter(u => {
        ...
        if (lower.includes('.com') || lower.includes('.org') || lower.includes('.net') || lower.includes('.co.')) {
            return false;
        }
        return true;
    }))];

    // ===== BANK NAMES (comprehensive) =====
    const bankKeywords = [
    ...
    for (let i = 0; i < bankKeywords.length; i += 2) {
        const keywords = bankKeywords[i];
        const name = bankKeywords[i + 1];
        if (Array.isArray(keywords)) {
            if (keywords.some(k => lowerText.includes(k))) {
                extracted.bankNames.push(name);
            }
        }
    }
    ...
    // ===== IDs (aggressive - capture any alphanumeric ID-like strings) =====
    // Employee/Badge ID
    const idPatterns = [
    ...
    idPatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(fullText)) !== null) {
            if (match[1] && match[1].length >= 2) {
                extracted.employeeId.push(match[1]);
            }
        }
    });
    ...
    casePatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(fullText)) !== null) {
            if (match[1] && match[1].length >= 2) {
                extracted.caseNumber.push(match[1]);
            }
        }
    });
    ...
    // ===== SUSPICIOUS KEYWORDS (comprehensive) =====
    const keywords = [
        // Urgency
    ...
    keywords.forEach(keyword => {
        if (lowerText.includes(keyword)) {
            extracted.suspiciousKeywords.push(keyword);
        }
    });
